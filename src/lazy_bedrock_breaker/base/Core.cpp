...
unsigned char handleDirection(unsigned char direction) {
    ...
    case 5:
        return 4; // 5 -> west
    default:
        return direction; // Invalid direction, return as is.
    }
}
...
void afterPlayerPlacedBlock(ll::event::PlayerPlacedBlockEvent& event) {
    ...
    Player&          player = event.self();
    const mce::UUID& uuid   = player.getUuid();
    if (!lazy_bedrock_breaker::mod().getPlayerDb()->get(uuid.asString())
        || !playerClickedPositions.count(uuid.asString()))
        return;
    ...
    TaskManager::addTask([&blockSource, facingPos, targetPos, pos, facing, waitTicks, &player, isCreative]() mutable {
